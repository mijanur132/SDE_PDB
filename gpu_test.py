...
if device.type == 'cuda':
 ...
 # Define the size of the matricesN = 100000  # Size of the square matrices
N =10000  # Size of the square matrices
 # Generate random matrices
if device.type == 'cuda':
    A = torch.randn(N, N).to(device)  # Random matrix A on GPU
    B = torch.randn(N, N).to(device)  # Random matrix B on GPU
else:
